...
function FinancialPerformance({ isArabic, onUnavailable }: { isArabic: boolean; onUnavailable: () => void }) {
  const months = isArabic
    ? ["فبراير 2026", "مارس 2026", "أبريل 2026", "مايو 2026", "يونيو 2026", "يوليو 2026"]
    : ["Feb 2026", "Mar 2026", "Apr 2026", "May 2026", "Jun 2026", "Jul 2026"];
  const revenue = [980, 1050, 1120, 1180, 1220, 1280];
  const expenses = [520, 545, 560, 600, 615, 645];
  const x = [70, 245, 420, 595, 770, 945];
  const revenueY = revenue.map((value) => 196 - ((value - 250) / 1250) * 150);
  const expensesY = expenses.map((value) => 196 - ((value - 250) / 1250) * 150);
  const revenuePoints = x.map((point, index) => `${point},${revenueY[index]}`).join(" ");
  const expensesPoints = x.map((point, index) => `${point},${expensesY[index]}`).join(" ");
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-base font-bold text-slate-950 dark:text-white">{isArabic ? "الأداء المالي" : "Financial Performance"}</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold outline-none dark:border-white/10 dark:bg-slate-900">
            <option>{isArabic ? "ريال سعودي" : "Saudi Riyal"}</option>
          </select>
          <div className="flex rounded-xl border border-slate-200 p-1 text-xs font-bold dark:border-white/10">
            <button onClick={onUnavailable} className="rounded-lg bg-indigo-600 px-3 py-1.5 text-white">{isArabic ? "6 أشهر" : "6 months"}</button>
            <button onClick={onUnavailable} className="rounded-lg px-3 py-1.5 text-slate-600 dark:text-slate-300">{isArabic ? "3 أشهر" : "3 months"}</button>
            <button onClick={onUnavailable} className="rounded-lg px-3 py-1.5 text-slate-600 dark:text-slate-300">{isArabic ? "سنة" : "Year"}</button>
          </div>
        </div>
      </div>

      <div className="mt-5 flex justify-center gap-6 text-xs font-medium text-slate-500">
        <span className="inline-flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-emerald-500" />{isArabic ? "الإيرادات" : "Revenue"}</span>
        <span className="inline-flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-indigo-500" />{isArabic ? "المصروفات" : "Expenses"}</span>
      </div>

      <div className="mt-3 overflow-x-auto">
        <svg viewBox="0 0 1010 255" role="img" aria-label={isArabic ? "مخطط الأداء المالي" : "Financial performance chart"} className="h-auto min-w-[720px] w-full">
          {[46, 76, 106, 136, 166, 196].map((y, index) => (
            <g key={y}>
              <line x1="55" x2="970" y1={y} y2={y} stroke="currentColor" className="text-slate-200 dark:text-white/10" strokeWidth="1" />
              <text x="45" y={y + 4} textAnchor="end" fontSize="11" fill="currentColor" className="text-slate-400">
                {["1.5M", "1.25M", "1M", "750K", "500K", "250K"][index]}
              </text>
            </g>
          ))}
          <line x1="55" x2="970" y1="214" y2="214" stroke="currentColor" className="text-slate-300 dark:text-white/20" />
          <polyline fill="none" stroke="#22c55e" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" points={revenuePoints} />
          <polyline fill="none" stroke="#6366f1" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" points={expensesPoints} />
          {x.map((point, index) => (
            <g key={point}>
              <circle cx={point} cy={revenueY[index]} r="4" fill="#22c55e" />
              <circle cx={point} cy={expensesY[index]} r="4" fill="#6366f1" />
              <text x={point} y={revenueY[index] - 10} textAnchor="middle" fontSize="11" fontWeight="700" fill="#16a34a">{revenue[index] >= 1000 ? `${(revenue[index] / 1000).toFixed(2)}M` : `${revenue[index]}K`}</text>
              <text x={point} y={expensesY[index] + 18} textAnchor="middle" fontSize="11" fontWeight="700" fill="#4f46e5">{expenses[index]}K</text>
              <text x={point} y="237" textAnchor="middle" fontSize="11" fill="currentColor" className="text-slate-500">{months[index]}</text>
            </g>
          ))}
        </svg>
      </div>

      <button onClick={onUnavailable} className="mt-2 text-xs font-semibold text-indigo-600">← {isArabic ? "تقرير الأداء المالي" : "Financial performance report"}</button>
    </article>
  );
}
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric, index) => (
          <article key={metric.label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
            ...
            <p className={`mt-2 text-xs font-semibold ${metric.tone === "positive" ? "text-emerald-600" : metric.tone === "warning" ? "text-amber-600" : "text-slate-500"}`}>{metric.change}</p>
          </article>
        ))}
      </section>

      <section className="grid gap-6 xl:grid-cols-[1.55fr_1fr]">
        <FinancialPerformance isArabic={isArabic} onUnavailable={onUnavailable} />
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
          ...
          <div className="mt-5 space-y-5">
            {recentActivity.map((activity) => (
              <div key={activity.title} className="flex gap-3"><span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-indigo-500" /><div><p className="text-sm leading-6">{activity.title}</p><p className="mt-1 text-xs text-slate-500">{activity.meta}</p></div></div>
            ))}
          </div>
        </article>
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
        ...
        <div className="overflow-x-auto">
          <table className={`min-w-full text-sm ${isArabic ? "text-right" : "text-left"}`}>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/10">{rows.map((row, index) => <tr key={index} className="hover:bg-slate-50/70 dark:hover:bg-white/[0.03]">{headers.map((header) => <td key={header} className="whitespace-nowrap px-5 py-4">{row[header]}</td>)}<td className="px-5 py-4"><button onClick={onUnavailable} className="font-bold text-indigo-600">{isArabic ? "عرض" : "View"}</button></td></tr>)}</tbody>
          </table>
        </div>
      </section>
    </div>
...
export default function DemoPage() {
  ...
  return (
    <main dir={isArabic ? "rtl" : "ltr"} lang={language} className="min-h-screen bg-slate-100 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="flex min-h-screen">
        ...
        <section className="min-w-0 flex-1">
          <header className="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur sm:px-7 dark:border-white/10 dark:bg-slate-950/90">
            ...
            <div className="flex items-center gap-2"><div className="flex rounded-xl border border-slate-200 p-1 text-xs font-bold dark:border-white/10"><button onClick={() => setLanguage("ar")} className={`rounded-lg px-3 py-1.5 ${isArabic ? "bg-indigo-600 text-white" : "text-slate-500"}`}>عربي</button><button onClick={() => setLanguage("en")} className={`rounded-lg px-3 py-1.5 ${!isArabic ? "bg-indigo-600 text-white" : "text-slate-500"}`}>EN</button></div><select value={role} onChange={(event) => setRole(event.target.value)} className="rounded-xl border border-slate-200 bg-transparent px-3 py-2 text-sm dark:border-white/10">{demoRoles.map((item) => <option key={item}>{item}</option>)}</select><div className="grid h-10 w-10 place-items-center rounded-full bg-indigo-100 font-bold text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300">A</div></div>
          </header>
          <div className="p-4 sm:p-7">
            <div className="mx-auto mb-6 max-w-2xl rounded-2xl border border-amber-300 bg-amber-50 px-5 py-3 text-center text-sm font-bold text-amber-800 shadow-sm dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300"><div>⚠ {isArabic ? "وضع العرض التجريبي · لا توجد عمليات حقيقية" : "Demo Mode · No real operations"}</div><div className="mt-1 text-xs font-normal opacity-80">{isArabic ? "جميع البيانات المعروضة تجريبية ولأغراض العرض فقط" : "All displayed data is fictional and for demonstration purposes only"}</div></div>
      ...
      {toastVisible && <div className={`fixed bottom-6 ${isArabic ? "left-6" : "right-6"} z-50 w-[min(22rem,calc(100vw-3rem))] rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl dark:border-white/10 dark:bg-slate-900`}><p className="font-bold">{isArabic ? "قيد التطوير" : "Under Development"}</p><p className="mt-1 text-sm text-slate-500">{isArabic ? "هذه الميزة قيد التطوير وستكون متاحة قريبًا." : "This feature is currently under development and will be available soon."}</p></div>}
    </main>
  );
}
